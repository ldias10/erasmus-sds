...
const CourseForm = ({ formData, onSubmit, setFormData, loading=false, buttonText="Upload" }: CourseFormProps) => {
  const [studyLevels, setStudyLevels] = useState<{ id: number; name: string; }[]>([]);
  const [fieldOfStudy, setFieldOfStudy] = useState<{ id: number; name: string; }[]>([]);
  ...
  const handleChangeSelect = (name: string, selectedOption: { id: number; name: string; } | null): void => {
    setFormData({
      ...formData,
      [name]: selectedOption ? selectedOption.id : undefined,
    });
  };
